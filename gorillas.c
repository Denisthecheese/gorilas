...
#define BUILDING_MIN_RELATIVE_HEIGHT     50        // Minimum height in % of the screenHeight
#define BUILDING_MAX_RELATIVE_HEIGHT    100        // Maximum height in % of the screenHeight
...
static Vector2 windowedScreenPosition;
...
static bool ballCollided = false;

static double currentTime = 0.0;
static double beginTurnTime = 0.0;
static double turnTimeElapsed = 0.0;
static double maximumTurnTime = 15;
...
int main(void)
{
    ...
    SetTargetFPS(60);
    windowedScreenPosition = GetWindowPosition();
...
void UpdateGame(void)
{
    ...
    if(IsKeyPressed('F')) 
    {
        ...
        else
        {
            ...
            SetWindowPosition(windowedScreenPosition.x, windowedScreenPosition.y);
        }
    ...
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            if(!ballOnAir) 
            {
                ballOnAir = UpdatePlayer(); // If we are aiming
            }
            else
            {
                ballCollided = UpdateBall();
                if(ballCollided)
                {
                    // Game over logic
                    ...
                    if (leftTeamAlive && rightTeamAlive)
                    {
                        ...
                        // Begin the new timer for the turn                        
                        beginTurnTime = GetTime();
        ...
        if (!gameOver)
        {
            ...
            if (!ballOnAir)
            {
                ...
                */
                ...
                DrawText(TextFormat("Time left in turn: %d\n", (int)(maximumTurnTime - turnTimeElapsed)), 20, 20, 20, BLACK);
...
static bool UpdatePlayer()
{
    //Turn Timer Logic
    currentTime = GetTime();
    turnTimeElapsed = currentTime - beginTurnTime;
    if(turnTimeElapsed > maximumTurnTime)
    {
        playerTurn = (playerTurn + 1) % MAX_PLAYERS; 
        beginTurnTime = currentTime;
        turnTimeElapsed = 0;
    }
